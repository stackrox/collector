...
runGCPCosTestViaSSH() {
  ...
  if gcloud compute ssh "$GCP_VM_NAME" --command 'pwd && docker version && ls -l /var/run/docker.sock && date && echo TOOLBOX && sudo toolbox --bind /var/run:/extrun --bind $(pwd):$(pwd) --bind /tmp:/exttmp $(pwd)/collector/.circleci/coshelp/oncos'" $DOCKER_USER $DOCKER_PASS" -- -t ; then
