...
ifdef BUILD_BUILDER_IMAGE
	docker buildx buildbuild0 --push \
...
image: collector unittest container-dockerfile $(MOD_VER_FILE) $(CURDIR)/$(COLLECTOR_BUILD_CONTEXT)/bundle.tar.gz
	...
	docker buildx buildbuild0 --push --build-arg collector_version="$(COLLECTOR_TAG)" \
...
shellcheck-all-dockerized:
	docker buildx buildbuild0 --push --platform=linux/ppc64le,linux/amd64 -f Dockerfile -t shellcheck-all $(CURDIR)/utilities/shellcheck-all
