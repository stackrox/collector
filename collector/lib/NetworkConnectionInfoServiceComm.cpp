...
std::unique_ptr<IDuplexClientWriter<sensor::NetworkConnectionInfoMessage>> NetworkConnectionInfoServiceComm::CommunicateOpenStream(std::function<void(const sensor::MsgToCollector*)> receive_func) {
  if (!context_) {
    ResetClientContext();
  }
