...
class INetworkConnectionInfoServiceComm {
 public:
  ...
  virtual std::unique_ptr<IDuplexClientWriter<sensor::NetworkConnectionInfoMessage>> CommunicateOpenStream(std::function<void(const sensor::MsgToCollector*)> receive_func) = 0;
};
...
class NetworkConnectionInfoServiceComm : public INetworkConnectionInfoServiceComm {
 public:
  ...
  std::unique_ptr<IDuplexClientWriter<sensor::NetworkConnectionInfoMessage>> CommunicateOpenStream(std::function<void(const sensor::MsgToCollector*)> receive_func) override;
