...
void NetworkStatusNotifier::OnRecvControlMessage(const sensor::MsgToCollector* msg) {
  if (!msg) {
    return;
  }
...
void NetworkStatusNotifier::Run() {
  ...
  while (thread_.PauseUntil(next_attempt)) {
    ...
    auto client_writer = comm_->CommunicateOpenStream([this](const sensor::MsgToCollector* msg) { OnRecvControlMessage(msg); });
