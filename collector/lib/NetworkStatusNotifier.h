...
class NetworkStatusNotifier : protected ProtoAllocator<sensor::NetworkConnectionInfoMessage> {
 public:
  ...
  NetworkStatusNotifier(std::string hostname, std::shared_ptr<IConnScraper> conn_scrapper, int scrape_interval, bool scrape_listen_endpoints,
                        bool turn_off_scrape,
                        std::shared_ptr<ConnectionTracker> conn_tracker,
                        std::shared_ptr<grpc::Channel> channel,
                        int64_t afterglow_period_micros,
                        bool use_afterglow)
      : hostname_(std::move(hostname)), conn_scraper_(conn_scrapper), scrape_interval_(scrape_interval), turn_off_scraping_(turn_off_scrape), scrape_listen_endpoints_(scrape_listen_endpoints), conn_tracker_(std::move(conn_tracker)), channel_(std::move(channel)), stub_(sensor::NetworkConnectionInfoService::NewStub(channel_)), afterglow_period_micros_(afterglow_period_micros), enable_afterglow_(use_afterglow) {
  }
