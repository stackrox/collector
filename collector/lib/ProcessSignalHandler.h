...
class ProcessSignalHandler : public SignalHandler {
 public:
  ProcessSignalHandler(sinsp* inspector, std::shared_ptr<grpc::Channel> channel, SysdigStats* stats, CollectorStats* collector_stats)
      : client_(std::move(channel)), formatter_(inspector), stats_(stats) { formatter_.SetCollectorStats(collector_stats); }
