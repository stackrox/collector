...
class ProcessSignalHandler : public SignalHandler {
 public:
  ProcessSignalHandler(std::string hostname, sinsp* inspector, std::shared_ptr<grpc::Channel> channel, SysdigStats* stats)
      : client_(std::move(hostname), std::move(channel)), formatter_(inspector), stats_(stats) {}
