...
bool GetSocketINodes(int dirfd, uint64_t pid, UnorderedSet<SocketInfo>* sock_inodes) {
  ...
  if (!fd_dir.valid()) {
    ...
    CLOG_THROTTLED(ERROR, std::chrono::seconds(1)) << "could not open fd directory";
    return false;
  }
...
bool ReadContainerConnections(const char* proc_path, std::shared_ptr<ProcessStore> process_store,
  ...
  if (!procdir.valid()) {
    ...
    CLOG_THROTTLED(ERROR, std::chrono::seconds(1)) << "Could not open " << proc_path << ": " << StrError();
    return false;
  }
  ...
  while (auto curr = procdir.read()) {
    ...
    if (!GetSocketINodes(dirfd, pid, &container_ns_sockets)) {
      ...
      CLOG_THROTTLED(ERROR, std::chrono::seconds(1)) << "Could not obtain socket inodes: " << StrError();
      continue;
    }
...
bool ReadProcessExe(const char* process_id, int dirfd, std::string& comm, std::string& exe_path) {
  ...
  if (nread <= 0 || nread >= ssizeof(buffer)) {
    ...
    CLOG_THROTTLED(ERROR, std::chrono::seconds(1)) << "Could not read 'exe' for " << process_id << ": " << StrError();
    return false;
  }
...
bool ReadProcessCmdline(const char* process_id, int dirfd, std::string& exe, std::string& args) {
  ...
  if (!cmdline.valid()) {
    ...
    CLOG_THROTTLED(ERROR, std::chrono::seconds(1)) << "Could not read 'cmdline' for " << process_id << ": " << StrError();
    return false;
  }
  bool did_exe = false;
