...
class Pipeline {
  using QueueTuple = std::tuple<std::shared_ptr<Queue<typename Nodes::InputType>>...>;
  struct Graph {
  ...
  static auto build_nodes(const std::tuple<std::shared_ptr<Queue<typename Nodes::InputType>>...>& queues,
