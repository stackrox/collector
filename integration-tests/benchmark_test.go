...
func (b *BenchmarkTestSuiteBase) StartPerfTools() {
	...
	if perf != "" {
		perf_image := "quay.io/rhacs-eng/collector-performance:" + GetQATag("perf")
		b.StartPerfContainer("perf", perf_image, perf)
	}

	if bpftrace != "" {
		bpftrace_image := "quay.io/rhacs-eng/collector-performance:" + GetQATag("bpftrace")
		b.StartPerfContainer("bpftrace", bpftrace_image, bpftrace)
	}

	if bcc != "" {
		bcc_image := "quay.io/rhacs-eng/collector-performance:" + GetQATag("bcc")
...
func (b *BenchmarkTestSuiteBase) RunInitContainer() {
	init_image := "quay.io/rhacs-eng/collector-performance:" + GetQATag("init")
	cmd := []string{
		"host-init",
