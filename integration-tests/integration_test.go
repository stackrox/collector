...
func TestRepeatedNetworkFlowNoAfteglow(t *testing.T) {
	// The timings of the curl commands are the same as above, but afterglow is turned off.
	// At each scrape interval there are two reports of inactive connections.
	// One is the server to client connection. The other is the client to sever connection.
...
func TestRepeatedNetworkFlowWithAfterglowExpiration(t *testing.T) {
	...
	// Again we see reports for the client to server and server to client connections.
	...
	repeatedNetworkFlowTestSuite := &RepeatedNetworkFlowTestSuite{
		...
		sleepBetweenIterations: 1,
		expectedReports: []bool{true, true, false, false, true, true, false, false},
	}
	suite.Run(t, repeatedNetworkFlowTestSuite)
}

func TestRepeatedNetworkFlowLongPauseNoAfterglow(t *testing.T) {
	// The timings of the curl commands are the same as in the test above, but now afterglow is disabled.
	// All connections are reported as being inactive. We still see client to server and server to client
	...
	repeatedNetworkFlowTestSuite := &RepeatedNetworkFlowTestSuite{
		...
		sleepBetweenCurlTime: 15,
		sleepBetweenIterations: 1,
		expectedReports: []bool{false, false, false, false},
	}
	suite.Run(t, repeatedNetworkFlowTestSuite)
}

func TestRepeatedNetworkFlowWithMultipleAfterglowExpirations(t *testing.T) {
	// This is similar to the two tests above, except that we have 3 curls instead of 2.
	...
	repeatedNetworkFlowTestSuite := &RepeatedNetworkFlowTestSuite{
		...
		sleepBetweenCurlTime: 15,
		sleepBetweenIterations: 1,
		expectedReports: []bool{true, true, false, false, true, true, false, false, true, true, false, false},
	}
	suite.Run(t, repeatedNetworkFlowTestSuite)
}
...
func GetNetworkActivity(lines []string) []bool {
	...
	//activePattern := "^Network.*(timestamp: nil Timestamp)$"
