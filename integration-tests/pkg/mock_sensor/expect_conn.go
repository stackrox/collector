...
func (s *MockSensor) ExpectConnections(t *testing.T, containerID executor.ContainerID, timeout time.Duration, expected ...types.NetworkInfo) bool {
...
loop:
	for {
		...
		case network := <-s.LiveConnections():
			if network.GetContainerId() != containerID.Short() {
				continue loop
			}
...
// have been received
func (s *MockSensor) ExpectConnectionsN(t *testing.T, containerID executor.ContainerID, timeout time.Duration, n int) []types.NetworkInfo {
...
loop:
	for {
		...
		case conn := <-s.LiveConnections():
			if conn.GetContainerId() != containerID.Short() {
				continue loop
			}
			if len(s.Connections(containerID)) == n {
				return s.Connections(containerID)
...
// does not matter.
func (s *MockSensor) ExpectSameElementsConnections(t *testing.T, containerID executor.ContainerID, timeout time.Duration, expected ...types.NetworkInfo) bool {
...
// until timeout or until all the events have been received.
func (s *MockSensor) ExpectEndpoints(t *testing.T, containerID executor.ContainerID, timeout time.Duration, expected ...types.EndpointInfo) bool {
...
// It does not consider the content of the events, just that a certain number
// have been received
func (s *MockSensor) ExpectEndpointsN(t *testing.T, containerID executor.ContainerID, timeout time.Duration, n int) []types.EndpointInfo {
...
func (s *MockSensor) WaitEndpointsN(t *testing.T, containerID executor.ContainerID, timeout time.Duration, n int) bool {
...
func (s *MockSensor) waitEndpointsN(t *testing.T, timeoutFn func(), containerID executor.ContainerID, timeout time.Duration, n int) []types.EndpointInfo {
...
loop:
	for {
		...
		case ep := <-s.LiveEndpoints():
			if ep.GetContainerId() != containerID.Short() {
				continue loop
			}
