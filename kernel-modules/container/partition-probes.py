...
def write_layer_manifests(probe_buckets, output_dir):
    ...
    def write_layer_file(name, probes=()):
        with open(name, 'w') as f:
            f.writelines("{0}\n".format(probe_info['name'] for probe_info in probes))

    if len(probe_buckets) == 0:
        # Write a zero-length bucket if there are no drivers
        write_layer_file(os.path.join(output_dir, "0"))

    for idx, probes in enumerate(probe_buckets):
        write_layer_file(os.path.join(output_dir, "{}".format(idx)), probes)
