...
int BPF_KSYSCALL(sys_enter_execve) {
  ...
  unsigned long filename_pointer = extract__syscall_argument(ctx, 0);
 ...
 */
  ...
  if (ret == 0) {
     ...
     */
    unsigned long argv = extract__syscall_argument(ctx, 1);
...
int BPF_PROG(t1_execve_x,
  ...
  if (ret == 0) {
    ...
    /* Parameter 16: env (type: PT_CHARBUFARRAY) */
    unsigned long envp = extract__syscall_argument(ctx, 2);
