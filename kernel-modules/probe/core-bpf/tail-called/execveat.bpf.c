...
int BPF_KSYSCALL(sys_enter_execveat) {
  ...
  s32 dirfd = (s32)extract__syscall_argument(ctx, 0);
  if (dirfd == AT_FDCWD) {
    dirfd = PPM_AT_FDCWD;
  }
  ...
  unsigned long pathname_pointer = extract__syscall_argument(ctx, 1);
  auxmap__store_charbuf_param(auxmap, pathname_pointer, MAX_PATH, USER);

  /* Parameter 3: flags (type: PT_FLAGS32) */
  unsigned long flags = extract__syscall_argument(ctx, 4);
 ...
 */
  ...
  if (ret == 0) {
     ...
     */
    unsigned long argv = extract__syscall_argument(ctx, 2);
...
int BPF_PROG(t1_execveat_x,
  ...
  if (ret == 0) {
    ...
    /* Parameter 16: env (type: PT_CHARBUFARRAY) */
    unsigned long envp = extract__syscall_argument(ctx, 3);
